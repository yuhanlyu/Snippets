...
Let $T(n) = \Theta(f(n))$. Assume that $T(n)$ is a constant for sufficiently small $n$. Derive $f(n)$ in the simplest formula for each of the following $T(n)$.
\begin{enumerate}
\item $T(n) = 5T(\frac{n}{5}) + n/\lg n$. \label{ABmethod:1} \school{[NCTU CSIE 93]}
\item $T(n) = T(\frac{n}{2} + \sqrt{n}) + n$. \label{ABmethod:2} \school{[NTU CSIE 103]}
\item $T(n) = 4T(\frac{n}{5}) + T(\frac{n}{4}) + n$. \label{ABmethod:3} \school{[NCTU BIOINFO 93]}
\end{enumerate}
\end{Exercise}
\begin{Answer}
...
\paragraph{Problem~\ref{ABmethod:2}} apply the Akra\--Bazzi method. Set $k = 1$, $a_1 = 1$, $b_1 = 1/2$, and solve $p = 0$. Hence, we get
\[ T(n) = \Theta( 1 (1 + \int_{1}^n (x / x) dx)) = \Theta(n). \]

\paragraph{Problem~\ref{ABmethod:3}} apply the Akra\--Bazzi method. Set $k = 2$, $a_1 = 4$, $b_1 = 1/5$, $a_2 = 1$, $b_2 = 1/4$, and solve $p \approx 1.03$. We get
...
Let $T(n) = \Theta(f(n))$. Assume that $T(n)$ is a constant for sufficiently small $n$. Derive $f(n)$ in the simplest formula for each of the following $T(n)$.
\begin{enumerate}
\item $T(n) = n + \frac{4}{n} \sum_{i=1}^{n-1} T(i)$. \label{full-history:1} \school{[NTU CSIE 90]}
\end{enumerate}
\end{Exercise}
\begin{Answer}
...
Let $T(n) = \Theta(f(n))$. Assume that $T(n)$ is a constant for sufficiently small $n$. Derive $f(n)$ in the simplest formula for each of the following $T(n)$.
\begin{enumerate}
\item $T(n) = \sqrt{n} T(\sqrt{n}) + \sqrt{n}$. \label{range-transformation:1} \school{[NCTU CSIE 93]}
\end{enumerate}
\end{Exercise}
\begin{Answer}
...
Let $T(n) = \Theta(f(n))$. Assume that $T(n)$ is a constant for sufficiently small $n$. Derive $f(n)$ in the simplest formula for each of the following $T(n)$.
\begin{enumerate}
\item $T(n) = 2T(\sqrt{n}) + \lg n$. \label{recursion-trees:1} \school{[NCKU CSIE 95]}
\item $T(n) = nT(\sqrt{n}) + n^2 \lg n$. \label{recursion-trees:2} \school{[NTU CSIE 98]}
\end{enumerate}
\end{Exercise}
\begin{Answer}
...
Let $T(n) = \Theta(f(n))$. Assume that $T(n)$ is a constant for sufficiently small $n$. Derive $f(n)$ in the simplest formula for each of the following $T(n)$.
\begin{enumerate}
\item $T(n) = T(\frac{n}{2}) + T(\sqrt{n}) + n$. \label{comparison:1} \school{[NCTU BIOINFO 93]}
\end{enumerate}
\end{Exercise}
\begin{Answer}
