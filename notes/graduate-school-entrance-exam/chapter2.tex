...
Since we know $d$ is between $1$ and $\sum_i S_i$, we can use binary search to find the minimum feasible $d$ value and the running time is $O(n \lg n)$.
Let $S(i, j)$ be $\sum_{k=i}^j S_k$.
The minimum $d$ value must be $S(i, j)$ for some $i$ and $j$.
Hence, it suffice to do binary search on $S(i, j)$.
%Let $S(i, j)$ be 
